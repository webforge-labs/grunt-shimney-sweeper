module.exports = function(grunt) {
  ...
  var less_regexp = new RegExp(".*?\\.less$", "i");
  var fileName_regexp = new RegExp(".+\\/((?:\\w|\\.)+)");
  ...
  var Sweeper = function() {
        ...
        var traverseDependencies = function(parentDep, rootDep) {
          _(parentDep.dependencies).forEach(function(dep) {
            ...
            if (dep.name.indexOf('shimney-') === 0) {
              // something like node_modules/path/to/dep (without a front (back)slash)
    ...
    this.sweepout = function (options, cb) {
      ...
      options.onTraverse = function (dep) {
        ...
        if (dep.shimName.indexOf('ui-') === 0) {
          grunt.file.recurse(dep.realPath, function (abspath, rootdir, subdir, filename) {
            if (subdir == 'img') { //copy images
              grunt.file.copy(abspath, packageDir + path.sep + options.img + path.sep + filename);
            } else if (css_regexp.test(filename)) { //copy css
              grunt.file.copy(abspath, packageDir + path.sep + options.css + path.sep + filename);
            } else if (less_regexp.test(filename)) { //copy less
              grunt.file.copy(abspath, packageDir + path.sep + options.less + path.sep + filename);
            }
          });
        }
